...
open class DropDown : UITextField{
    ...
    @IBInspectable public var arrowColor: UIColor = .darkGray {
        didSet{
...
extension DropDown: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let fon  = UIFont(name: "Helvetica", size: 14)
        let text = "\(dataArray[indexPath.row])" as NSString
        let size = text.size(withAttributes: [NSAttributedString.Key.font:fon!])
        cell?.textLabel?.frame = CGRect(x: 0, y: 0, width:  size.width, height: size.height)//CGRectMake(0, 0, size.width, size.height)
...
class Arrow: UIView {
    ...
    var arrowColor:UIColor = .darkGray {
        didSet{
