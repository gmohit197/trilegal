...
- (void)layoutSubviews
{
    ...
    if (_subtitle) {
        ...
        _substrLabel.text = _substring;
        if (_subtitleLabel.hidden) {
            _subtitleLabel.hidden = NO;
            _substrLabel.hidden = NO;
        }
    } else {
        if (!_subtitleLabel.hidden) {
            _subtitleLabel.hidden = YES;
            _substrLabel.hidden = YES;
        }
    }
    ...
    if (_subtitle) {
        ...
        CGFloat substrHeight = self.substrLabel.font.lineHeight;
        ...
        _substrLabel.frame = CGRectMake(
                                          self.preferredSubstrOffset.x,
                                          (_subtitleLabel.fs_bottom-self.preferredSubstrOffset.y) - (_subtitleLabel.fs_height-_subtitleLabel.font.pointSize)+self.preferredSubstrOffset.y,
                                          self.contentView.fs_width,
                                          substrHeight
                                          );
    } else {
...
- (void)configureAppearance
{
    ...
    if (_substring) {
        ...
        titleFont = self.calendar.appearance.subtitleFont;
        if (![titleFont isEqual:
              _substrLabel.font]) {
            _substrLabel.font = titleFont;
        }
    }
...
OFFSET_PROPERTY(preferredSubstrOffset, PreferredSubstrOffset, _appearance.substrOffset); 
...
{
    if (![_substring isEqualToString:substring]) {
        BOOL diff = (substring.length && !_substring.length) || (_substring.length && !substring.length);
